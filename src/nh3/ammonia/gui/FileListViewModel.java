      ...
      case COL_WARNINGS: {
        ...
        if (focusingPatterns.isSet()) {
          return (int) this.fWarnings.get(row)
              .stream()
              .filter(warning -> focusingPatterns.contains(warning.pattern.mergedID))
              .count();
        } else if (logKeyPatterns.isSet() && !metricsPatterns.isSet()) {
          return this.fWarnings.get(row)
              .stream()
              .filter(warning -> logKeyPatterns.contains(warning.pattern.mergedID))
              .count();
        } else if (!logKeyPatterns.isSet() && metricsPatterns.isSet()) {
          return this.fWarnings.get(row)
              .stream()
              .filter(warning -> metricsPatterns.contains(warning.pattern.mergedID))
              .count();
        } else if (logKeyPatterns.isSet() && metricsPatterns.isSet()) {
          return this.fWarnings.get(row)
              .stream()
