...
public class StringUtility {
  ...
  static public List<String> split(final String nameText) {
    ...
    try (final BufferedReader reader = new BufferedReader(new StringReader(nameText))) {
      while (true) {
  ...
  static public SVNURL getSVNURL(final String repository, final String path) {
    ...
    try {
      if (repository.startsWith("http://")) {
        ...
        throw new IllegalStateException("illegal URL (, which is specified by \"-repo\" option)");
      }
