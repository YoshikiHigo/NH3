...
public class FBMeter {
  ...
  public static void main(final String[] args) {
    ...
    final List<String> xmls = FBParserConfig.getInstance()
        .getFBRESULTS();
    ...
    for (int i = 2; i < xmls.size(); i++) {
      ...
      final Set<BugInstance> earlierBugs = new HashSet<>(bugInstances.get(earlierXMLFile));
      final Set<BugInstance> latterBugs = new HashSet<>(bugInstances.get(latterXMLFile));
      ...
      final Transition pair =
          new Transition(earlierXMLFile, latterXMLFile, survivingBugs, addedBugs, removedBugs);
  ...
  static private int countBugInstances(final Set<BugInstance> instances, final String type) {
    int count = 0;
  ...
  static private void printInCSV(final String path, final List<Transition> transitions,
    ...
    for (final BugPattern pattern : BugPattern.getBugPatterns()) {
      ...
      text.append(pattern.type)
          .append(", ")
          .append(pattern.getAverageRank())
          .append(", ")
          .append(pattern.getAveragePriority())
          .append(", ")
    ...
    for (final Transition transition : transitions) {
      for (final Entry<BugPattern, StringBuilder> entry : patternsMap.entrySet()) {
        ...
        final Set<BugInstance> instances = BugInstance.getBugInstances(bugs, pattern);
        text.append(", ")
            .append(Integer.toString(instances.size()));
      }
    }
    ...
    try (final PrintWriter writer =
      ...
      for (final Transition transition : transitions) {
        title.append(",")
            .append(StringUtility.getName(transition.latterXMLFile));
      }
  ...
  static private void printInXLSX(final String path, final List<Transition> transitions) {
    ...
    try (final Workbook book = new XSSFWorkbook();
      ...
      survivingStyle.setFillForegroundColor(IndexedColors.ROSE.getIndex());
      ...
      nocareStyle.setFillForegroundColor(IndexedColors.GREY_25_PERCENT.getIndex());
      ...
      typeRow.createCell(0)
          .setCellValue("TYPE");
      rankRow.createCell(0)
          .setCellValue("RANK");
      priorityRow.createCell(0)
          .setCellValue("PRIORITY");
      categoryRow.createCell(0)
          .setCellValue("CATETORY");
      metricRow.createCell(0)
          .setCellValue("METRIC");
      ...
      for (final BugPattern pattern : BugPattern.getBugPatterns()) {
        ...
        typeRow.createCell(titleColumn)
            .setCellValue(pattern.type);
        sheet.addMergedRegion(new CellRangeAddress(0, 0, titleColumn, titleColumn + 5));
        rankRow.createCell(titleColumn)
            .setCellValue(pattern.getRankText());
        sheet.addMergedRegion(new CellRangeAddress(1, 1, titleColumn, titleColumn + 5));
        priorityRow.createCell(titleColumn)
            .setCellValue(pattern.getPriorityText());
        sheet.addMergedRegion(new CellRangeAddress(2, 2, titleColumn, titleColumn + 5));
        categoryRow.createCell(titleColumn)
            .setCellValue(pattern.category);
        sheet.addMergedRegion(new CellRangeAddress(3, 3, titleColumn, titleColumn + 5));
        metricRow.createCell(titleColumn)
            .setCellValue("#-surviving-bugs");
        metricRow.createCell(titleColumn + 1)
            .setCellValue("#-removed-bugs");
        metricRow.createCell(titleColumn + 2)
            .setCellValue("#-added-bugs");
        metricRow.createCell(titleColumn + 3)
            .setCellValue("%-surviving-bugs");
        metricRow.createCell(titleColumn + 4)
            .setCellValue("%-solved-old");
        metricRow.createCell(titleColumn + 5)
            .setCellValue("%-solved-new");
      ...
      for (final Transition pair : transitions) {
        ...
        row.createCell(0)
            .setCellValue(StringUtility.getName(pair.latterXMLFile));

        int dataColumn = 1;
        for (final BugPattern pattern : BugPattern.getBugPatterns()) {

          final int numberOfSurvivingBugs = countBugInstances(pair.survivingBugs, pattern.type);
          final int numberOfAddedBugs = countBugInstances(pair.addedBugs, pattern.type);
          final int numberOfRemovedBugs = countBugInstances(pair.removedBugs, pattern.type);

          float ratioOfSurviving =
              (float) numberOfSurvivingBugs / (float) (numberOfSurvivingBugs + numberOfRemovedBugs);
          float ratioOfSolvedOld =
              (float) numberOfRemovedBugs / (float) (numberOfSurvivingBugs + numberOfRemovedBugs);
          float ratioOfSolvednew =
              (float) numberOfRemovedBugs / (float) (numberOfSurvivingBugs + numberOfAddedBugs);

          row.createCell(dataColumn)
              .setCellValue(numberOfSurvivingBugs);
          row.createCell(dataColumn + 1)
              .setCellValue(numberOfRemovedBugs);
          row.createCell(dataColumn + 2)
              .setCellValue(numberOfAddedBugs);
          row.createCell(dataColumn + 3)
              .setCellValue(ratioOfSurviving);
          row.createCell(dataColumn + 4)
              .setCellValue(ratioOfSolvedOld);
          row.createCell(dataColumn + 5)
              .setCellValue(ratioOfSolvednew);

          {
            final AtomicInteger number = new AtomicInteger(numberOfSurvivingBugs);
...

      {
        ...
        for (final BugPattern pattern : BugPattern.getBugPatterns()) {
          ...
          final int numberOfSurvivingBugs = survivingBugs.get(pattern.type)
              .get();
          final int numberOfRemovedBugs = removedBugs.get(pattern.type)
              .get();
  ...
  static class Transition {
    ...
    Transition(final String earlierXMLFile, final String latterXMLFile,
        final Set<BugInstance> survivingBugs, final Set<BugInstance> addedBugs,
        final Set<BugInstance> removedBugs) {
